...
async function transcribeWithAssemblyAI(file, onStatus) {
  const headers = { 'authorization': ASSEMBLYAI_KEY }
  ...
  // Step 2 — Request transcription with word timestamps
  onStatus('Starting transcription...')
  const transcriptRes = await fetch(TRANSCRIPT_URL, {
    method: 'POST',
    headers: { ...headers, 'Content-Type': 'application/json' },
    body: JSON.stringify({ audio_url: upload_url, word_boost: [], format_text: true })
  })
  ...
  while (attempts < 60) {
    ...
    const pollRes = await fetch(`${TRANSCRIPT_URL}/${id}`, { headers })
    ...
    if (result.status === 'completed') {
      ...
      const words = result.words && result.words.length > 0
        ...
        : result.text.trim().split(/\s+/).map((word, i, arr) => ({
            word,
            start: parseFloat((i * (result.audio_duration / arr.length)).toFixed(2)),
            end: parseFloat(((i + 1) * (result.audio_duration / arr.length)).toFixed(2))
          }))
      ...
      <AnimatePresence>
        {transcriptionError && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <br />
            <span className="text-red-300/60 text-xs mt-1 block">Please try uploading again.</span>
          </motion.div>
        )}
      </AnimatePresence>
