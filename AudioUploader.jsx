...
export default function AudioUploader({ onNext }) {
  ...
  const onDrop = useCallback(async (acceptedFiles) => {
    ...
    try {
      ...
      const result = await transcribeWithWhisper(file, (msg) => {
        setStatusMsg(msg)
      })
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm text-white/70 flex items-center gap-2">
                ...
                {statusMsg || 'Transcribing with Whisper AI...'}
              </span>
            ...
            <p className="text-white/30 text-xs font-mono mt-3">
              This may take 20-40 seconds on first use while model loads...
            </p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {transcriptionError && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <br />
            <span className="text-red-300/60 text-xs mt-1 block">Try uploading again — model may still be loading.</span>
          </motion.div>
        )}
      </AnimatePresence>
