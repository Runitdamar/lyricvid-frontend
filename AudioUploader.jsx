...
export default function AudioUploader({ onNext }) {
  ...
  const onDrop = useCallback(async (acceptedFiles) => {
    ...
    setIsTranscribing(true); setUploadProgress(10)
    try {
      const formData = new FormData()
      formData.append('audio', file)
      setUploadProgress(30)
      const response = await fetch(`${BACKEND_URL}/api/transcribe`, { method: 'POST', body: formData })
      setUploadProgress(80)
      if (!response.ok) { const err = await response.json(); throw new Error(err.error || 'Transcription failed') }
      const data = await response.json()
      setUploadProgress(100); setTranscript(data.transcript); setWords(data.words); setIsTranscribing(false)
    } catch (err) {
      setTranscriptionError(err.message); setIsTranscribing(false); setUploadProgress(0)
    }
  }, [])

  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop, accept: { 'audio/*': ['.mp3', '.wav', '.m4a', '.ogg', '.flac'] }, maxSize: 25 * 1024 * 1024, multiple: false
  })
        ...
        {isTranscribing && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <p className="text-white/30 text-xs font-mono mt-3">
              {uploadProgress < 30 ? 'Uploading...' : uploadProgress < 80 ? 'Whisper AI reading every word...' : 'Almost done...'}
            </p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {transcriptionError && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <br /><span className="text-red-300/60 text-xs mt-1 block">Check your backend is running and HF token is set.</span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {audioUrl && !isTranscribing && !transcriptionError && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex justify-center pt-4">
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }} onClick={onNext} className="btn-primary text-lg py-4 px-12">
              Next: Edit Lyrics →
            </motion.button>
          </motion.div>
        )}
