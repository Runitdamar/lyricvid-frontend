...
export default function CanvasPreview({ onNext, onBack }) {
  ...
  useEffect(() => {
    ...
    } else if (store.backgroundType !== 'image') {
  ...
  useEffect(() => {
    ...
    return () => cancelAnimationFrame(animRef.current)
      ...
      store.shadowColor, store.shadowBlur, store.lineTransition])
  ...
  const togglePlay = () => {
    ...
    if (isPlaying) {
      ...
      if (audio.currentTime >= store.audioDuration - 0.1) audio.currentTime = 0
      audio.play()
  ...
  const handleSeek = (e) => {
    ...
    const t = ((e.clientX - r.left) / r.width) * store.audioDuration
    if (audioRef.current) audioRef.current.currentTime = Math.min(t, store.audioDuration)
  }

  const progressPct = store.audioDuration ? Math.min(100, (currentTime / store.audioDuration) * 100) : 0
  ...
  return (
    <div className="space-y-4">
      <div className="text-center mb-4">
        ...
        <p className="text-white/40 text-sm font-mono">9:16 • {store.audioDuration.toFixed(1)}s clip</p>
      </div>
      ...
      <audio
        ...
        onTimeUpdate={() => {
          if (audioRef.current && audioRef.current.currentTime >= store.audioDuration) {
      ...
      <div className="flex justify-center">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
          ...
          <div className="absolute bottom-3 right-3 bg-black/40 rounded-full px-2 py-0.5">
            <span className="font-mono text-xs text-white/30">9:16</span>
          </div>
        </motion.div>
      </div>
      ...
      <div className="glass rounded-xl p-4 border border-white/10 space-y-3">
        ...
        <div className="w-full h-2 bg-white/10 rounded-full cursor-pointer" onClick={handleSeek}>
          <motion.div className="h-2 rounded-full bg-gradient-to-r from-brand-600 to-brand-400 transition-none"
        ...
        <div className="flex flex-wrap gap-2">
          {[store.theme, store.lineTransition, store.textEffect].map(label => (
            <span key={label} className="bg-white/5 border border-white/10 rounded-full px-3 py-1 text-xs font-mono text-white/40 capitalize">{label}</span>
          ))}
        </div>
      </div>
